...
static TCHAR childclass[] = _T("Settings");
...
int registerAllClass(HINSTANCE hInstance) {
	...
	WNDCLASSEXW settingswcex;

	settingswcex.cbSize = sizeof(WNDCLASSEX);

	settingswcex.style = CS_HREDRAW | CS_VREDRAW;
	settingswcex.lpfnWndProc = SettingsWndProc;
	settingswcex.cbClsExtra = 0;
	settingswcex.cbWndExtra = 0;
	settingswcex.hInstance = hInstance;
	settingswcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON1));
	settingswcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	settingswcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	settingswcex.lpszMenuName = 0;
	settingswcex.lpszClassName = childclass;
	settingswcex.hIconSm = LoadIcon(settingswcex.hInstance, MAKEINTRESOURCE(IDI_ICON1));
	if (!RegisterClassEx(&settingswcex)) {
		MessageBox(NULL, _T("call to RegisterClassEX(child) failed."), _T("TheEditor"), NULL);
		return 1;
	}
...
void createsettingswindows() {
	HWND hwnd = CreateWindowW(childclass, TEXT("Setting the hash function number"),
		WS_OVERLAPPEDWINDOW,
	...
	//add controls
	hWndedit = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("Edit"), TEXT("8"),
		WS_CHILD | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 400,
...
LRESULT CALLBACK SettingsWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case EN_CHANGE:
	{
		...
		if (GetWindowTextLength(hWndedit) != 1)
		{

			break;
		}
		GetWindowText(hWndedit, tmp, 2);
		
		if (tmp[0] == '1') {
			MenuCommandcoref.sethashfunction_num(1);
		}
		else if (tmp[0] == '2') {
			MenuCommandcoref.sethashfunction_num(2);
		}
		else if (tmp[0] == '3') {
			MenuCommandcoref.sethashfunction_num(3);
		}
		else if (tmp[0] == '4') {
			MenuCommandcoref.sethashfunction_num(4);
		}
		else if (tmp[0] == '5') {
			MenuCommandcoref.sethashfunction_num(5);
		}
		else if (tmp[0] == '6') {
			MenuCommandcoref.sethashfunction_num(6);
		}
		else if (tmp[0] == '7') {
			MenuCommandcoref.sethashfunction_num(7);
		}
		else if(tmp[0] == '8') {
		//else if (tmp == L"8")
...
int textcontent::getwordrange(_textrange &textrangevar) {
	return SendMessage(re, EM_GETTEXTRANGE, 0, (LPARAM)&textrangevar);
}
